...
var rawRoutes = {{= routes }}; // 记录所有前端渲染页面的路由规则
var routes = [];
// 把routes.path转化为正则
// 如 /detail/:id => /^\/detail\/[^\/]+\/?$/
for (var i = 0; i < rawRoutes.length; i++) {
    if (rawRoutes[i].pagename) {
        rawRoutes[i].pathRegExp =
            new RegExp('^' + rawRoutes[i].path.replace(/\/:[^\/]*/g, '/[^\/]+') + '\/?');
        routes.push(rawRoutes[i]);
    }
}
...
// 为路由添加默认入口继续匹配（添加后缀可配置）
// 如 /list => /list/index.html
...
// 根据routers.json获取pagename，从而寻找入口
// 如：/detail/123 => /detail.html
var findEntryHtml = function (originalUrl) {
    var url = new URL(originalUrl);

    for (var i = 0; i < routes.length; i++) {
        if (routes[i].pathRegExp.test(url.pathname)) {
            url.pathname = '/' + routes[i].pagename + '.html';
            break;
        }
    }
